...
export class DomainWarningFeedRepository {
  ...
  constructor(
    @InjectModel(DomainWarningFeed.name) private domainWarningFeedModel: Model<DomainWarningFeedDocument>
  ) {}

  async findByDomainId(domainId: number): Promise<DomainWarningFeed | null> {
    return this.domainWarningFeedModel.findOne({
      domainId,
  ...
  async findAll(options?: { skip?: number; limit?: number }): Promise<DomainWarningFeed[]> {
    let query = this.domainWarningFeedModel.find({ browserType: BrowserType.WEBKIT });
  ...
  async update(domainId: number, data: Partial<DomainWarningFeed>): Promise<DomainWarningFeed | null> {
    return this.domainWarningFeedModel.findOneAndUpdate(
      { domainId, browserType: BrowserType.WEBKIT },
      data,
      { new: true }
    ).exec();
  }
  ...
  async remove(domainId: number): Promise<DomainWarningFeed | null> {
    return this.domainWarningFeedModel.findOneAndDelete({
      domainId,
      browserType: BrowserType.WEBKIT
    }).exec();
  }
